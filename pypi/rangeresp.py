...
class HTTPFile(io.IOBase):
    ...
    Note: Server MUST support range requests for this to work. The class will attempt to determine the content length and range support on initialization. If the server does not support range requests, it will raise a ValueError when trying to read.
    """
    ...
    def _evict_if_needed(self) -> None:
        """Evict the least recently used blocks from the cache if the cache exceeds the maximum allowed blocks."""
        while len(self._cache) > self.max_blocks:
            evicted_block, _ = self._cache.popitem(last=False)
            self._log(f"Evicted block {evicted_block}")
    ...
    def _fetch_block(self, block_index: int) -> bytes:
        ...
        response = self.client.get(
            self.url,
            headers={"Range": f"bytes={block_start}-{block_end}"},
            timeout=5.0,
            follow_redirects=True,
        )
...
def main() -> None:
    url = "https://upload.wikimedia.org/wikipedia/commons/d/d1/Ulica_Czerniakowska%2C_pa%C5%BAdziernik_2022_03.jpg"
    print(stat(url))

...
# ---------------------------------------------------------------------------
# Valid-source generator layer
# ---------------------------------------------------------------------------
# The full generator above is useful for fuzzing raw AST shapes, but many Python
# AST combinations are structurally legal objects while still not legal Python
# programs, e.g. ``break`` outside loops, ``return`` at module level, ``await``
# outside async functions, invalid assignment targets, duplicated function
# arguments, or expression nodes in places where the compiler rejects them.
#
# The functions below intentionally override the broad generators with a smaller
# syntax-valid subset.  They still generate varied Python code, but every public
# source-generation entry point validates with ``compile()`` before returning.

_VALIDATION_ATTEMPTS = 1000
_SAFE_MAX_DEPTH = 3


def _unique_identifiers(prefix: str, count: int) -> list[str]:
    names: list[str] = []
    seen: set[str] = set()
    while len(names) < count:
        name = generate_identifier(prefix)
        if name not in seen:
            seen.add(name)
            names.append(name)
    return names
...
def _safe_name(prefix: str = "x", ctx: ast.expr_context | None = None) -> ast.Name:
    return ast.Name(id=generate_identifier(prefix), ctx=ctx or ast.Load())


def _safe_target() -> ast.expr:
    if random.random() < 0.8:
        return _safe_name("x", ast.Store())
    return ast.Tuple(
        elts=[_safe_name("x", ast.Store()) for _ in range(random.randint(1, 3))],
        ctx=ast.Store(),
    )


def _safe_arguments(depth: int = 0) -> ast.arguments:
    posonly_count = random.randint(0, 2)
    args_count = random.randint(0, 3)
    kwonly_count = random.randint(0, 2)
    vararg_count = random.randint(0, 1)
    kwarg_count = random.randint(0, 1)
    total = posonly_count + args_count + kwonly_count + vararg_count + kwarg_count
    names = iter(_unique_identifiers("arg", total))

    posonlyargs = [
        ast.arg(arg=next(names), annotation=None, type_comment=None)
    ...
    positional_count = len(posonlyargs) + len(args)
    default_count = random.randint(0, positional_count)
    defaults = [generate_Constant() for _ in range(default_count)]
    kw_defaults = [random.choice([None, generate_Constant()]) for _ in kwonlyargs]
...
def _safe_slice(depth: int) -> ast.expr | ast.slice:
    if random.random() < 0.5:
        return _safe_expr(depth + 1)
    return ast.Slice(
        lower=random.choice([None, _safe_expr(depth + 1)]),
        upper=random.choice([None, _safe_expr(depth + 1)]),
        step=random.choice([None, _safe_expr(depth + 1)]),
    )


def _safe_expr(depth: int = 0) -> ast.expr:
    if depth >= _SAFE_MAX_DEPTH:
        return random.choice([generate_Constant(), _safe_name("x")])
    ...
    if choice == "binop":
        return ast.BinOp(
            left=_safe_expr(depth + 1),
            op=random.choice([ast.Add(), ast.Sub(), ast.Mult(), ast.Div()]),
            right=_safe_expr(depth + 1),
        )
    if choice == "boolop":
        return ast.BoolOp(
            op=generate_boolop(),
            values=[_safe_expr(depth + 1) for _ in range(random.randint(2, 3))],
        )
    if choice == "compare":
        ...
        return ast.Compare(
            ...
            comparators=[_safe_expr(depth + 1) for _ in range(n)],
        )
    if choice == "unary":
        return ast.UnaryOp(
            op=random.choice([ast.Not(), ast.UAdd(), ast.USub()]),
            operand=_safe_expr(depth + 1),
        )
    if choice == "ifexp":
        return ast.IfExp(
            test=_safe_expr(depth + 1),
            body=_safe_expr(depth + 1),
            orelse=_safe_expr(depth + 1),
        )
    if choice == "list":
        return ast.List(
            elts=[_safe_expr(depth + 1) for _ in range(random.randint(0, 4))],
            ctx=ast.Load(),
        )
    if choice == "tuple":
        return ast.Tuple(
    ...
    if choice == "set":
        return ast.Set(
            elts=[_safe_expr(depth + 1) for _ in range(random.randint(1, 4))]
        )
    if choice == "dict":
        n = random.randint(0, 4)
        return ast.Dict(
            keys=[_safe_expr(depth + 1) for _ in range(n)],
            values=[_safe_expr(depth + 1) for _ in range(n)],
        )
    if choice == "call":
        return ast.Call(
            func=random.choice(
                [
                    _safe_name("fn"),
                    ast.Attribute(
                        value=_safe_name("obj"),
                        attr=generate_identifier("method"),
                        ctx=ast.Load(),
                    ),
                ]
            ),
            args=[_safe_expr(depth + 1) for _ in range(random.randint(0, 3))],
            keywords=[
                ast.keyword(arg=generate_identifier("kw"), value=_safe_expr(depth + 1))
                for _ in range(random.randint(0, 2))
            ],
        )
    if choice == "attribute":
        return ast.Attribute(
            value=_safe_name("obj"), attr=generate_identifier("attr"), ctx=ast.Load()
        )
    if choice == "subscript":
        return ast.Subscript(
            value=_safe_name("x"), slice=_safe_slice(depth), ctx=ast.Load()
        )
    return ast.Lambda(args=_safe_arguments(depth + 1), body=_safe_expr(depth + 1))
...
def generate_arguments() -> ast.arguments:
    return _safe_arguments()


def _safe_simple_stmt(in_function: bool = False) -> ast.stmt:
    choices = ["pass", "expr", "assign", "annassign", "import"]
    if in_function:
    ...
    if choice == "pass":
        return ast.Pass()
    if choice == "expr":
        return ast.Expr(value=_safe_expr())
    if choice == "assign":
        return ast.Assign(
            targets=[_safe_target()], value=_safe_expr(), type_comment=None
        )
    if choice == "annassign":
        return ast.AnnAssign(
            target=_safe_name("x", ast.Store()),
            annotation=_safe_name("T"),
            value=random.choice([None, _safe_expr()]),
            simple=1,
        )
    if choice == "return":
        return ast.Return(value=random.choice([None, _safe_expr()]))
    return ast.Import(
        names=[
            ast.alias(
                name=generate_module_name(),
...
def _safe_block(depth: int = 0, *, in_function: bool = False) -> list[ast.stmt]:
    if depth >= _SAFE_MAX_DEPTH:
        return [_safe_simple_stmt(in_function=in_function)]
    body = [
        _safe_stmt(depth + 1, in_function=in_function)
        for _ in range(random.randint(1, MAX_LIST_SIZE))
    ]
    return body or [ast.Pass()]


def _safe_stmt(depth: int = 0, *, in_function: bool = False) -> ast.stmt:
    if depth >= _SAFE_MAX_DEPTH:
        return _safe_simple_stmt(in_function=in_function)

    choices = ["simple", "if", "while", "for", "function", "class", "try", "with"]
    choice = random.choice(choices)
    if choice == "simple":
        return _safe_simple_stmt(in_function=in_function)
    if choice == "if":
        return ast.If(
            test=_safe_expr(),
            body=_safe_block(depth + 1, in_function=in_function),
            orelse=_safe_block(depth + 1, in_function=in_function)
            if random.random() < 0.5
            else [],
        )
    if choice == "while":
    ...
    if choice == "function":
        return ast.FunctionDef(
            name=generate_identifier("fn"),
            args=_safe_arguments(depth + 1),
            body=_safe_block(depth + 1, in_function=True),
            decorator_list=[],
            returns=random.choice([None, _safe_name("T")]),
            type_comment=None,
            type_params=[],
        )
    if choice == "class":
        return ast.ClassDef(
            ...
            bases=[],
            keywords=[],
            body=_safe_block(depth + 1, in_function=False),
            decorator_list=[],
            type_params=[],
        )
    if choice == "try":
    ...
    return ast.With(
        items=[
            ast.withitem(
                context_expr=_safe_expr(),
                optional_vars=random.choice([None, _safe_target()]),
            )
        ],
        body=_safe_block(depth + 1, in_function=in_function),
        type_comment=None,
...
def generate_FunctionDef() -> ast.FunctionDef:
    return cast(
        ast.FunctionDef,
        _safe_stmt(choice_depth := 0)
        if False
        else ast.FunctionDef(
            name=generate_identifier("fn"),
            args=_safe_arguments(1),
            body=_safe_block(1, in_function=True),
            decorator_list=[],
            returns=random.choice([None, _safe_name("T")]),
            type_comment=None,
            type_params=[],
...
# def generate_Module() -> ast.Module:
#     return ast.Module(body=_safe_block(), type_ignores=[])


def generate_valid_module(*, attempts: int = _VALIDATION_ATTEMPTS) -> ast.Module:
    """Return a random ``ast.Module`` that Python accepts with ``compile()``."""
    ...
    for _ in range(attempts):
        ...
        try:
            with warnings.catch_warnings():
                ...
                compile(tree, "<ast_generator>", "exec")
        except (SyntaxError, ValueError, TypeError) as exc:
            last_error = exc
            continue
        return tree
    raise RuntimeError(
        f"could not generate a valid module after {attempts} attempts"
    ) from last_error


def sample_source(*, attempts: int = _VALIDATION_ATTEMPTS) -> str:
    """Generate Python source that parses and compiles successfully."""
    last_error: Exception | None = None
    for _ in range(attempts):
        try:
            tree = generate_valid_module(attempts=1)
            source = ast.unparse(tree)
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", SyntaxWarning)
                warnings.simplefilter("ignore", DeprecationWarning)
                compile(source, "<ast_generator_source>", "exec")
        except (SyntaxError, ValueError, TypeError, RuntimeError) as exc:
            last_error = exc
            continue
        return source
    raise RuntimeError(
        f"could not generate valid source after {attempts} attempts"
    ) from last_error

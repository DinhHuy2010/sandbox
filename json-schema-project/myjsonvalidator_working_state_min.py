...
class SchemaError(Exception):0
class ValidationError(Exception):0
class ValidationResult:
	def __init__(A,evaluated_properties=_A,evaluated_items=_A):A.evaluated_properties=set(evaluated_properties or());A.evaluated_items=set(evaluated_items or())
	def merge(A,other):
		B=other
		if B is _A:return A
		A.evaluated_properties.update(B.evaluated_properties);A.evaluated_items.update(B.evaluated_items);return A
